# Resource module to create an EC2 instance for frontend.
module "frontend" {
...
resource "aws_ami_from_instance" "frontend" {
  ...
  # Server will be stopped only when the instance is stopped.
...
resource "null_resource" "frontend_delete" {
  ...
  provisioner "local-exec" {
    command = "aws ec2 terminate-instances --instance-id ${module.frontend.id}"
  }
...
# Resource to create the auto scaling group policy for frontend.
...
# Resource to create the listener rule in frontend load balancer.
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["web-${var.environment}.${var.project_name}.${var.zone_name}"]
    }
  }
}
